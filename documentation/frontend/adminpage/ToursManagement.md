...
- `TourFormPage`
  ...
  - Persists through the backend:
    - New tour: `POST /api/tours`
    - Existing tour: `PATCH /api/tours/:tourId`
...
- Add new tour fields: update `Tour` type (`src/types/global.d.ts`), extend form state defaults, and include fields in backend create/update payloads.
