...
## Data Flow
- Loads **Besas**, **Tours**, and **Bookings** from `GET /api/admin/schedule`.
...
- Edit booking form with save via `PATCH /api/admin/bookings/:bookingId` and delete via `DELETE /api/admin/bookings/:bookingId`; `isSaving`/`isDeleting` guard double-submits.
- Displays BESA assignments and time ranges, formatting to 12h when needed.

## Extending
- Add filters by tour or BESA: derive from the loaded `bookings` + `tours` response.
